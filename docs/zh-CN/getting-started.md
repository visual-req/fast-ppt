...
## 第 1.5 步：先判断这套 PPT 属于哪一类

在执行 `/fppt:outline` 之前，最好先明确这套 PPT 的类型。  
当前 skill 固定支持四类：
...
- 类型会直接影响章节结构
- 类型会影响图页比例和 layout 选择
- 类型选错后，detail 阶段通常越补越乱
...
2. [PPT 类型说明](ppt-types.md)：四种类型分别适合什么场景
3. [项目结构](structure.md)：各目录分别放什么
4. [布局类型库](layouts.md)：每页能用哪些 layout
5. [操作手册](manual.md)：实际工作时怎么配合使用
